...
ABLATION_VARIANTS: List[Tuple[str, str | None]] = [
    ...
    ("sem_clima", "climatico"),
    ("sem_temporal_profundo", "temporal_profundo"),
...
def build_single_prediction_report(
    *,
    ...
    lines = [
        ...
        "",
        f"- Jogo unico: {row['nums']} | score_final={float(row['score_final']):.6f} | score_decisao_protegida={float(row.get('score_decisao_protegida', 0)):.6f} | score_transicao={float(row.get('score_transicao', 0)):.6f} | score_climatico={float(row.get('score_climatico', 50)):.6f} | score_temporal_profundo={float(row.get('score_temporal_profundo', 50)):.6f}",
        ...
        "9. Usa score temporal profundo com dia da semana, ultimos 15/30 dias, bimestre, trimestre e semestre.",
        "10. Nao divide a previsao entre dois jogos.",
        "",
...
def _result_row(
    *,
    ...
    row = {
        ...
        "score_temporal_profundo": float(best.get("score_temporal_profundo", 0.0)),
