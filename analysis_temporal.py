...
# Gera análises temporais e expõe um "perfil temporal" reutilizável pelo model.py/rounds.py.
#
...
def add_time_buckets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "data_sorteio" not in df.columns:
        raise ValueError("df precisa ter coluna data_sorteio.")
...
def build_temporal_profile(
    ...
    *,
    ...
    def add_component(name: str, p_vec: np.ndarray, n_eff: float) -> None:
        nonlocal logp, wsum
        # peso cresce com o tamanho do bucket (evita bucket minúsculo dominar)
